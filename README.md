...
Код основан на материалах 2-й лабораторной работы
...
- **Исключения:** конструктор от массива с `count < 0` выбрасывает `const char*` (не `IndexOutOfRange`)
...
**Примечание:** используются флаги `-std=c++17 -stdlib=libc++` (для Clang с libc++).
